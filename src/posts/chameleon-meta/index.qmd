...
**Late fushion** Done at the internal layers of the model (e.g. OpenFlamingo, LLaMA-Adapter-V2)
...
![**Image 5:** Simplified architectural summary for tokenised early fusion - taken from Wadekar et.al](early_fusion_encoder.png)
...
- For images: trained a new image tokenizer based on [Gafni et.al (2022)](https://arxiv.org/abs/2203.13131) which encodes encodes a 512 × 512 image into 1024 discrete tokens from a codebook of size 8192

> Based on [OAI's pricing page](https://openai.com/api/pricing/) (as of 5 June 2024), one image is ~170 tokens in GPT-4o.

- For text: BPE tokenzier over a subset of training data, with a vocabulary size of 65K
...
2. Masking tokens from other modalities when exlcuslively generating for a particular modality (e.g. no text tokens when doing image-only generation)
3. Token-based iamge generation is a fixed-sized block
